...
function FlowNodeCard({ node, status }: { node: FlowNode; status: FlowStatus }) {
  ...
  return (
    ...
    >
      <div
        className={cn(
          "flex h-10 items-center gap-2 rounded-lg border px-3 text-sm font-semibold shadow-line transition",
          status === "pending" && "border-line bg-white text-muted",
          status === "running" && "border-brand/30 bg-brandLight text-ink shadow-soft",
          status === "success" && "border-brand/20 bg-brandLight/70 text-ink",
          status === "error" && "border-coral/30 bg-coral/10 text-coral",
        )}
      >
        <span
          className={cn(
            "grid h-6 w-6 shrink-0 place-items-center rounded-md",
            status === "pending" && "bg-canvas text-muted/65",
            status === "running" && "bg-brand text-white",
            status === "success" && "bg-brand/15 text-brand",
            status === "error" && "bg-coral/15 text-coral",
          )}
        >
  ...
  return (
    <section className="mt-4 overflow-hidden rounded-lg border border-line bg-surface shadow-soft">
      <div className="flex items-center justify-between gap-3 border-b border-line px-4 py-3">
        ...
        <div className="rounded-full bg-canvas px-2.5 py-1 text-xs text-muted">LangGraph</div>
      </div>
          ...
          >
            <defs>
              <marker
                ...
                refY="4"
              >
                <path d="M0 0 L8 4 L0 8 Z" fill="rgba(99,115,111,0.58)" />
              </marker>
            </defs>
            {connectors.map((path) => (
              <path
